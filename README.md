...
*   **Variable Handling:** Supports single-letter variables mapped automatically to memory offsets
*   **Pointer Operations:** Allows direct memory writing using the `*d = v;` syntax.
*   **Control Flow:** Implements basic `w(x){ ... }` while-loops, which can also be utilized for minimalist conditional logic.
*   **I/O Operations:** Can trigger DOS interrupts for character output via the `p x;` command.
*   **Bootstrapping:** The compiler is capable of processing its own logic to generate a new functional binary.
...
*   `build_y_minus.py`: The Python builder script that contains the raw, commented hex assembly and generates the final `.com` binary.
*   `self_compile.y-`: The source code for the compiler, written entirely in Y-Minus.
